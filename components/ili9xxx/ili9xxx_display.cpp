...
void ILI9XXXDisplay::fill(Color color) {
  uint8_t color332 = display::ColorUtil::color_to_332(
...
void ILI9XXXDisplay::fill_internal_(uint8_t color) {
  ...
  this->set_addr_window_(0, 0, this->get_width_internal(),
                         this->get_height_internal());
  this->start_data_();

  while (rem > 0) {
    size_t sz = rem <= sizeof(transfer_buffer_)
                    ? rem
                    : sizeof(transfer_buffer_); // NOLINT
...
void HOT ILI9XXXDisplay::draw_absolute_pixel_internal(int x, int y,
                                                      Color color) {
...
// should return the total size: return this->get_width_internal() *
// this->get_height_internal() * 2 // 16bit color values per bit is huge
...
uint32_t ILI9XXXDisplay::buffer_to_transfer_(uint32_t pos, uint32_t sz) {
  ...
  for (uint32_t i = 0; i < sz; ++i) {
    uint16_t color;
    if (this->buffer_color_mode_ == BITS_8) {
      color = display::ColorUtil::color_to_565(
          display::ColorUtil::rgb332_to_color(*src++));
